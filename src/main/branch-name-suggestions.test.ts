...
import { extractSlug, runAgent } from './branch-name-suggestion-agent'
...
describe('branch name suggestions', () => {
  it('sanitizes and caps untrusted agent output before git validation', () => {
    const slug = extractSlug('"../../Feature: Clean Up User Flow With A Very Long Tail????"')

    expect(slug).toBe('feature-clean-up-user-flow-with-a-very-long-tail'.slice(0, 48))
    expect(slug.length).toBeLessThanOrEqual(48)
    expect(slug).not.toContain('/')
    expect(slug).not.toContain('..')
  })

  it('caps agent stdout before returning output', async () => {
    mockAgentOutput('a'.repeat(20_000))

    const output = await runAgent('codex', 'name this branch', 'diff', '/repo/wt')
  ...
  it('marks pushed branches as skipped so they are not renamed later', async () => {
    ...
    expect(meta.branchNameSuggestion).toMatchObject({
      status: 'skipped',
      failureReason: 'branch already has an upstream or remote'
    })
  })

  it('skips branches with any upstream before asking an agent', async () => {
    const meta = makeMeta(makeSuggestion())
    const store = makeStore(meta)
    mockGit((args) => {
      if (args[0] === 'branch') {
  ...
  it('detects branch changes before applying a persisted suggestion', async () => {
    const meta = makeMeta(
      makeSuggestion({
        status: 'suggested',
        suggestedBranch: 'kaylee/better-branch-name'
    ...
    mockGit((args) => {
      if (args[0] === 'branch' && args[1] === '--show-current') {
        return { stdout: 'kaylee/user-renamed-it\n', stderr: '' }
      }
      return { stdout: '', stderr: '' }
    })
    ...
    expect(result).toEqual({
      ok: false,
      error: 'The branch changed before the suggestion could be applied.'
    })
    expect(meta.branchNameSuggestion).toMatchObject({
      status: 'skipped',
      failureReason: 'branch changed before apply'
    })
    expect(gitExecFileAsync).not.toHaveBeenCalledWith(
      expect.arrayContaining(['-m']),
      expect.anything()
    )
  })

  it('refuses to apply after the branch gains an upstream or remote ref', async () => {
    const meta = makeMeta(
      makeSuggestion({
        status: 'suggested',
        suggestedBranch: 'kaylee/better-branch-name'
    ...
    expect(result).toEqual({
      ok: false,
      error: 'This branch now has an upstream or remote branch, so Orca left it unchanged.'
    })
    expect(meta.branchNameSuggestion).toMatchObject({
      status: 'skipped',
      failureReason: 'branch already has an upstream or remote'
  ...
  it('refuses to apply when the suggested branch now conflicts', async () => {
    const meta = makeMeta(
      makeSuggestion({
        status: 'suggested',
        suggestedBranch: 'kaylee/better-branch-name'
    ...
    const store = makeStore(meta)
    vi.mocked(getBranchConflictKind).mockResolvedValue('remote')
    mockGit((args) => {
      if (args[0] === 'branch' && args[1] === '--show-current') {
        return { stdout: 'kaylee/original-name\n', stderr: '' }
      }
    ...
    expect(result).toEqual({
      ok: false,
      error: 'The suggested branch name now conflicts with another branch.'
    })
    expect(meta.branchNameSuggestion).toMatchObject({
      status: 'failed',
