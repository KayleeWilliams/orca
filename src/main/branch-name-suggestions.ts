...
function pathLeaf(pathValue: string): string {
  return getRuntimePathBasename(pathValue)
}
...
async function branchHasRemoteOrUpstream(
  worktreePath: string,
  branchName: string
): Promise<boolean> {
  return !!(await getUpstream(worktreePath)) || (await remoteBranchExists(worktreePath, branchName))
}
...
export async function maybeSuggestBranchNameAfterAgentDone(args: {
  ...
  try {
    ...
    if (await branchHasRemoteOrUpstream(worktreePath, branch)) {
      markSuggestion(args.store, args.worktreeId, suggestion, {
        status: 'skipped',
